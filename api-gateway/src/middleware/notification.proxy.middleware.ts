...
export const notificationServiceProxy = createProxyMiddleware({
  ...
  onProxyReqWs: (proxyReq, req, socket, options, head) => {
    ...
    // No authentication needed here - notification service handles it via first message
    logger.debug("[Notification Proxy] WebSocket upgrade request", {
      url: req.url,
    });
    // Just pass through - let the notification service handle authentication
  },
