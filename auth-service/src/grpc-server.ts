...
const authServiceImpl: UntypedServiceImplementation = {
 register: async (call: grpc.ServerUnaryCall<authProto.RegisterRequest, authProto.RegisterResponse>, callback: grpc.sendUnaryData<authProto.RegisterResponse>) => {
    try {
      logger.info('Received gRPC register request', { request: call.request.toObject() });
      const response = await authController.register(call.request);
      callback(null, response);
    } catch (err: any) {
      logger.error('gRPC register error', { error: err.message, stack: err.stack });
      const statusCode = err instanceof CustomError ? err.status : 500;
      callback({
        code: statusCode === 400 ? grpc.status.INVALID_ARGUMENT : grpc.status.INTERNAL,
...
server.addService(AuthServiceService as unknown as ServiceDefinition<UntypedServiceImplementation>, authServiceImpl);
